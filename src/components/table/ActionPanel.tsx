    ...
    <div className="bg-gray-950 border-t border-gray-800 p-3 space-y-3">
      ...
      {canCall && requiredEquity !== null && requiredEquity !== undefined && (
        <div className={`text-xs px-2 py-1.5 border font-sans leading-tight ${
          ...
          Call <span className="font-mono font-bold">{callAmount.toFixed(1)}</span> into a <span className="font-mono font-bold">{(pot + callAmount).toFixed(1)}</span> pot
          <span className="mx-1.5 text-gray-600">|</span>
          Need <span className={`font-mono font-bold ${potOddsColor}`}>{formatPercent(requiredEquity)}</span> to break even
          {heroEquity !== null && heroEquity !== undefined && (
            <>
              <span className="mx-1.5 text-gray-600">|</span>
              You have <span className={`font-mono font-bold ${evColor}`}>{formatPercent(heroEquity)}</span>
            ...
            <div className="flex gap-1 flex-wrap">
              {quickBets.map(b => (
                ...
                >
                  ...
                  <span className="text-[10px] font-mono text-gray-300">{b.amount.toFixed(1)} big blinds</span>
                </button>
              ))}
            </div>
          )}
